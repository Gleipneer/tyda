...
- [x] POST /api/posts/{id}/interpret (AI-tolkning, kräver OPENAI_API_KEY och inloggning med JWT)
- [x] GET /api/interpret/status
- [x] JWT (Bearer) efter `POST /api/auth/login` och `POST /api/users`; skriv-API för poster och post–begrepp kräver token
- [x] Begreppsbiblioteket är utökat via migration 001–016 (körs via `run_migration_utf8.py`)
- [x] Symbolmatcher med kluster, synonymer, böjningsformer
- [x] Frontend: Begrepp i fokus, auto-match, AI-tolkning på postdetalj
- [x] Frontend: session (användare + JWT) i localStorage (`tyda.session`), `Byt användare`, lokala utkast per användare
...
- Migrationer: 001–016 (bl.a. auth-kolumner 015, uppdateringstrigger 016)
